...
class TestAdminCountries:
    ...
    def test_export_returns_vbwd_envelope(self, admin_headers):
        """GET /export returns the VBWD-standard downloadable JSON envelope."""
        response = requests.get(
            f"{self.BASE_URL}/admin/countries/export",
            headers=admin_headers,
            timeout=5,
        )
        assert response.status_code == 200
        ...
        assert envelope["vbwd_export"] == "countries"
        assert envelope["version"] == 1
        assert isinstance(envelope["countries"], list)
        assert envelope["countries"], "export should not be empty after seeding"
        row = envelope["countries"][0]
        assert set(row) == {"code", "name", "is_enabled", "position"}

    def test_export_requires_auth(self):
        """GET /export without a token is rejected."""
        response = requests.get(f"{self.BASE_URL}/admin/countries/export", timeout=5)
        assert response.status_code in (401, 403)

    def test_import_upserts_countries(self, admin_headers):
        """POST /import upserts an exported envelope by code (idempotent)."""
        ...
        # Re-importing the exact export updates every row, creates none.
        response = requests.post(
            f"{self.BASE_URL}/admin/countries/import",
            json=exported,
            headers=admin_headers,
            timeout=10,
        )
        ...
        assert body["updated"] == len(exported["countries"])

    def test_import_rejects_malformed(self, admin_headers):
        """POST /import with a non-countries payload returns 400."""
        response = requests.post(
            f"{self.BASE_URL}/admin/countries/import",
            json={"vbwd_export": "plans", "countries": []},
            headers=admin_headers,
            timeout=5,
        )
        assert response.status_code == 400

    def test_import_requires_manage_permission(self, user_headers):
        """A non-admin user cannot import countries."""
        response = requests.post(
            f"{self.BASE_URL}/admin/countries/import",
            json={"countries": []},
            headers=user_headers,
