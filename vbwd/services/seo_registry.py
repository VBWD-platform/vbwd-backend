"""Registry of sitemap providers (plugin-contributed) — core-agnostic seam.

Core's ``/sitemap.xml`` aggregates every registered provider's entries. Core
declares no provider of its own (it names no content domain); a content plugin
(e.g. cms) registers a provider in ``on_enable``. With nothing registered the
aggregator returns ``[]`` — the route still serves a valid, empty urlset
(Liskov null default), never a crash.
