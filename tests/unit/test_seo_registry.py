"""S47.1 core seam — sitemap-provider registry (agnostic).

Core ships ``ISeoSitemapProvider`` + ``SitemapEntry`` + an aggregator that
returns ``[]`` when no provider is registered (Liskov null default). Core
declares no providers itself; plugins register theirs.
"""
import pytest
