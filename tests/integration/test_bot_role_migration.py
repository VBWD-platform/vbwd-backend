"""Migration coverage for the core ``BOT`` enum value (S60).

Verifies the ``20260607_1000_add_bot_role`` migration adds ``BOT`` to the
native Postgres ``userrole`` enum, is idempotent (up/down/up), and that
its downgrade is the documented no-op (Postgres cannot drop an enum
value).

Runs against a throwaway database so it never touches the shared test DB
or its alembic version table.
"""
import os
import uuid
...
def throwaway_db():
    ...
    db_name = f"bot_role_mig_{uuid.uuid4().hex[:10]}"
...
def _load_migration():
    ...
    path = os.path.abspath(
        os.path.join(
            ...
            "alembic",
            "versions",
            "20260607_1000_add_bot_role.py",
        )
    )
    spec = importlib.util.spec_from_file_location("bot_role_migration", path)
...
def test_bot_value_absent_before_migration(throwaway_db):
    engine = create_engine(throwaway_db)
    try:
        assert "BOT" not in _enum_values(engine)
    finally:
        engine.dispose()


def test_migration_adds_bot_value_and_is_idempotent(throwaway_db):
    _run_migration_upgrade(throwaway_db)

    engine = create_engine(throwaway_db)
    try:
        assert "BOT" in _enum_values(engine)
    finally:
        engine.dispose()
