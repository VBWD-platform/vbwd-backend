.PHONY: up down build test test-unit test-integration test-integration-keep-data test-coverage seed-test-data cleanup-test-data reset-demo seed-rbac populate-ghrm logs shell clean lint pre-commit pre-commit-quick
...
# Seed default RBAC roles + permissions (ungated, idempotent)
seed-rbac:
	docker compose exec api flask --app "vbwd:create_app()" seed-rbac
