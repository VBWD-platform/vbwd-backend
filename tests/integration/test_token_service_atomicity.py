"""S138.0 — TokenService is ONE unit of work (live-DB integration).

Three properties, all previously untrue (1 and 2 from Inc 1, 3 from Inc 2):
...
3. **``complete_purchase`` marks the purchase and credits under one commit.** It
   used to ``save()`` the COMPLETED status — which commits — before crediting,
   so a vetoed credit left the purchase durably COMPLETED with no tokens.
...
BUNDLE_TOKEN_AMOUNT = 250


@pytest.fixture
def pending_purchase(app, user_with_balance):
    """A committed PENDING token-bundle purchase for the balance-holding user."""
    from vbwd.extensions import db

    with app.app_context():
        bundle = TokenBundle(
            id=uuid4(),
            name=f"Atomicity bundle {uuid4().hex[:8]}",
            token_amount=BUNDLE_TOKEN_AMOUNT,
            price=9.99,
        )
        ...
        db.session.commit()
        purchase_id, bundle_id = purchase.id, bundle.id
        try:
            yield purchase_id
        finally:
            db.session.rollback()
            db.session.query(TokenBundlePurchase).filter(
                TokenBundlePurchase.id == purchase_id
            ).delete()
            db.session.query(TokenBundle).filter(TokenBundle.id == bundle_id).delete()
            db.session.commit()


def _persisted_purchase_status(purchase_id):
    from vbwd.extensions import db

    db.session.expire_all()
    return db.session.get(TokenBundlePurchase, purchase_id).status


class TestCompletePurchaseIsAtomic:
    """S138.0 Inc 2 — the purchase status and its credit commit together.

    ``complete_purchase`` used to ``save()`` the COMPLETED status (which
    commits) *before* crediting: a vetoed credit left the purchase durably
    COMPLETED with no tokens.
    """

    def test_a_vetoed_credit_leaves_the_purchase_pending_and_the_balance_untouched(
        self, app, user_with_balance, pending_purchase
    ):
        from vbwd.extensions import db
        ...
        with app.app_context():
            ...
            with pytest.raises(RuntimeError, match="bookkeeping failed"):
                service.complete_purchase(pending_purchase)

            assert (
                _persisted_purchase_status(pending_purchase) == PurchaseStatus.PENDING
            )
            balance, transaction_count = _persisted_state(user_with_balance)
            assert balance == INITIAL_BALANCE
            assert transaction_count == 0

    def test_a_successful_purchase_still_completes_and_credits(
        self, app, user_with_balance, pending_purchase
    ):
        from vbwd.extensions import db
        ...
        with app.app_context():
            ...
            assert (
                _persisted_purchase_status(pending_purchase) == PurchaseStatus.COMPLETED
            )
            balance, transaction_count = _persisted_state(user_with_balance)
            assert balance == INITIAL_BALANCE + BUNDLE_TOKEN_AMOUNT
            assert transaction_count == 1
