"""Core LLM adapters — one provider contract, two implementations (S97.0).

Extracted import-clean from the cms-ai LoopForge adapter so core owns the
dual-provider client without importing a plugin (core stays agnostic). The
``LlmAdapter`` interface is the narrow, Liskov-substitutable contract every
provider honours: a structured-JSON ``generate(...)`` and a plain-text
``chat(...)``. Two concrete adapters drive the providers via their official
Python SDKs:
...
The provider SDKs are imported **lazily** inside the build methods, so this
module imports clean even where ``openai`` / ``anthropic`` are not installed
(the unit tests drive a fake adapter, no SDK, no network). Both adapters route
every JSON call through the shared validate/repair/retry loop and surface any
SDK/transport failure as :class:`AdapterError`. The api_key is never echoed
into an error or return value.
"""
...
from vbwd.llm.json_io import request_valid_json
...
class LlmAdapter(ABC):
    """One provider surface (generate + chat); subclasses implement it."""
    ...
    def chat(
        ...
        *,
        ...
        """Return the model's plain-text reply to a multi-turn message list.

        Raises :class:`AdapterError` on SDK/transport failure.
        """
        raise NotImplementedError
...
def _normalize_anthropic_base_url(endpoint: str) -> str:
    ...
    ``https://api.anthropic.com/v1/`` endpoint (correct for the OpenAI-compatible
    adapter but breaks the native SDK -> ``/v1/v1/messages`` -> 404). Strip a
    trailing ``/v1`` so copied config still works. An empty endpoint stays empty
    (the SDK then uses its own default).
    """
...
def _run_repair_loop(call, *, json_schema, max_attempts) -> dict:
    """Drive the shared JSON repair/retry loop, normalising any leak to LlmError."""
    try:
...
def _build_json_tool(json_schema: Optional[dict]) -> dict:
    ...
    return {
        ...
        "description": "Return the requested fields as a single JSON object.",
...
def _extract_anthropic_text(response: Any) -> str:
    """Pull the plain assistant text out of an Anthropic chat response."""
    content_blocks = getattr(response, "content", None)
    if not content_blocks:
        raise AdapterError("Anthropic response had no content")
    first_block = content_blocks[0]
